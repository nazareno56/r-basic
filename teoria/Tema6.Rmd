...
Tambien recibe el nombre de tabla de contingencia.
```{r}
table(x)
...
```{r}
barplot(table(Sexo,Respuestas), legend.text = TRUE, col = c("lightblue", "pink"))
```

## Diagrama de barras - Tabla bidimensional

```{r}
barplot(table(Sexo,Respuestas), beside=TRUE, legend.text=TRUE, col = c("lightblue", "pink"))
```
...
barplot(table(Respuestas,Sexo), beside=TRUE, names=c("Men", "Women"), 
        col=c("firebrick1","darkolivegreen1"), legend.text=c("No","Yes"))
```
...
plot(table(Sexo,Respuestas), main="Gráfico de mosaico de las variables
     \"Sexo\" y \"Respuestas\"", col=c("firebrick1","darkolivegreen1"))
```
