...
- `df[n:m,n:m]`: para extraer “trozos” del data frame por filas y columnas (funciona exactamente igual que en matrices) donde $n$ y $m$ pueden definirse como:
    - intervalos
    - condiciones
    - números naturales
    - no poner nada
...
students = read.table("../data/StudentsData", 
                      col.names = c("technicalDisciplines","aptitude","maths",
                                    "language","generalKnowledge"))
head(students,8)
```
...
```{r}
Curso =      c("Curso 1","Curso 2","Curso 2","Curso 1","Curso 2","Curso 1")
Algebra =    c(        1,        2,        0,        5,        4,        6)
Analysis =   c(        3,        3,        2,        7,        9,        5)
Statistics = c(        4,        5,        4,        8,        8,        9)
grades = data.frame(Curso = Curso, Alg = Algebra, An = Analysis, Stat = Statistics,
                    stringsAsFactors = TRUE)
row.names(grades) = c("Maria","Gabriel","Agustin","Federico","Ana","Jimena")
head(grades, 5)
```
