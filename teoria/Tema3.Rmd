...
## Valores Na
- ```sum(x)```: nos restonar NA
```{r}
x = c(1,2,3,4,5,6,7,8,9,NA,NA,10)
sum(x)
```
- ```sum(x, na.rm = TRUE)```: ignora los NA
```{r}
sum(x, na.rm = TRUE)
```
## is.na()
- ```is.na(x)```: nos retorna un vector de booleanos

- Posicion en donde los valores son NA
```{r}
which(is.na(x))
```
...
- La funcion cumsum no admite ```na.rm = TRUE```. 
- Se debe realizar de la siguiente forma
```{r}
cumsum(x[!is.na(x)])
```

- Eliminar valores NA de un vector
```{r}
cumsum(na.omit(x))
```
...
## Determinante de un matriz con valores complejos
- No se encuentra definido para complejos en R
```{r, eval=FALSE}
M = matrix(c(3-2i, 5+3i, 1+2i, 2-1i), nrow=2, byrow=TRUE)
det(M)
```
```Error in determinant.matrix(x, logarithm = TRUE, ...) : 'determinant' not currently defined for complex matrices```

- Puede ser calculado, haciendo uso de la propiedad: det(M) = producto de sus autovalores
```{r}
prod(eigen(M)$values)
```
